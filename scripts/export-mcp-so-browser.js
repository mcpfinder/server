...
async function goToNextPage(currentPage) {
    ...
    const nextButtons = document.querySelectorAll(`a[href*="page=${nextPage}"]`);
    if (nextButtons.length > 0) {
        nextButtons[0].click();
    } else {
        // Try to find buttons with "Next" text
        const allButtons = [...document.querySelectorAll('button, a')];
        const nextButton = allButtons.find(btn => btn.textContent.trim().toLowerCase().includes('next'));
        if (nextButton) {
            nextButton.click();
        } else {
